...
value_df <- df %>% select(MntWines, MntFruits, MntMeatProducts, MntFishProducts, MntSweetProducts, MntGoldProds, NumDealsPurchases) %>%
  filter(NumDealsPurchases > 1) %>% select(-NumDealsPurchases) %>% 
  remove_rownames %>%
  na.omit %>%
  rename()
...
```{r}
no_k = 4 # number of clusters for clustering methods 
