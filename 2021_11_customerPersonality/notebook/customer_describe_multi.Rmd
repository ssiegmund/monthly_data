---
title: "describe bivariate for customer personality analysis data"
...
```{r}
name = c('')
tmp_df <- df %>% select(-ID, -Education, -Marital_Status, -Dt_Customer) %>% na.omit()
...
```{r}
name = c('')
tmp_df <- df %>% select(-ID, -Education, -Marital_Status, -Dt_Customer) %>% na.omit() %>%  sample_frac(0.3)
...
## bivariate numeric Income over numeric Year_Birth

- 

```{r}
# two variables, continuous x, continuous y, show trend and distribution
name = c('Year_Birth', 'Income')
...
p1 <- tmp_df %>%
  ggplot(aes(x = x, y = y)) +
    ...
    geom_quantile(alpha = 0.4, color = "green") +
...
## bivariate numeric Income over categorical Education

- 

```{r}
# two variables, continuous/discrete x, categorical y, show trend and distribution
name = c('Education', 'Income')
tmp_df <- df %>% rename(x = name[1], y = name[2]) %>% select(x, y) %>% mutate(x = fct_infreq(x)) %>% 
  add_count(x, y)
...
## bivarite categorical Education over categorical Marital_Status

-

```{r}
name = c('Marital_Status', 'Education')
