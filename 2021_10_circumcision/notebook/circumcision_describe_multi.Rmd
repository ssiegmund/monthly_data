---
title: "describe bivariate for Estimation of global male circumcision"
...
```{r}
name = c('')
tmp_df <- df %>% select(-country_territory, -alpha_2_country_code, -alpha_3_country_code, -intermediate_region, -total_males_circumcized, -basis) %>% na.omit()
...
## bivariate numeric male_circumcision_percent over numeric cia_total_males

- 

```{r}
# two variables, continuous x, continuous y, show trend and distribution
name = c('male_circumcision_percent', 'cia_total_males')
tmp_df <- df %>% rename(x = male_circumcision_percent, y = cia_total_males) %>% select(x, y) %>% add_count(x, y)
...
## bivariate numeric male_circumcision_percent over categorical region

- 

```{r}
# two variables, continuous/discrete x, categorical y, show trend and distribution
name = c('male_circumcision_percent', 'region')
tmp_df <- df %>% rename(x = region, y = male_circumcision_percent) %>% select(x, y) %>% mutate(x = fct_infreq(x)) %>% 
  add_count(x, y)
...
## bivarite categorical region over categprocal sub_region

-

```{r}
name = c('region', 'sub_region')
tmp_df <- df %>% rename(x = region, y = sub_region) %>% select(x, y) 
